...
function TreeItem<T = any>({
  ...
  const mergedStyle = {
    ...propStyle,
    "--tree-padding": `${(item.getItemMeta().level + indentOffset) * indent}px`,
  } as React.CSSProperties;
  ...
  return (
    ...
    >
      {item.isFolder() &&
        (toggleIconType === "plus-minus" ? (
          item.isExpanded() ? (
            <Minus aria-hidden="true" className="text-muted-foreground size-3.5" strokeWidth={1} />
          ) : (
            <Plus aria-hidden="true" className="text-muted-foreground size-3.5" strokeWidth={1} />
          )
        ) : (
          <ChevronDown
            aria-hidden="true"
