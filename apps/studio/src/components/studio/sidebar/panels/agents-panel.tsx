...
export function AgentsPanel({
  ...
  async function handleSend() {
    ...
    setInput("");
    setPendingAttachments([]);
    await sendMessage(activeSession.id, content, attachments);
  }
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-background text-xs">
      {/* 1. Header Toolbar: Session Switcher + Prompt Selector + Actions */}
      <div className="flex shrink-0 flex-col gap-1.5 border-b border-border/60 bg-muted/20 p-2.5">
        <div className="flex items-center justify-between gap-1.5">
            ...
            <DropdownMenuTrigger
              render={
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 max-w-[180px] gap-1.5 px-2 text-xs font-medium"
                >
          ...
          {/* Quick Actions (New chat, Clear, Delete) */}
                ...
                <Message key={m.id} align={isUser ? "end" : "start"} className="gap-2">
                  ...
                  <MessageContent className="max-w-[85%]">
                    ...
                    {(() => {
                      ...
                      const replaceAction = actions?.find((a) => a.action === "replace_document");
                      const insertCount =
                            ...
                            <div className="mt-1.5 flex flex-col gap-2 rounded-xl border border-primary/30 bg-primary/5 p-2.5">
                              ...
                              <div className="flex flex-wrap items-center justify-between gap-1.5">
                                <div className="flex flex-wrap items-center gap-1.5 text-[10px]">
                                  {replaceAction && replaceAction.action === "replace_document" ? (
                                    <Badge
                                      variant="outline"
