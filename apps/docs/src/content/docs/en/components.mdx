...
<Aside type="note" title="Build-time validation">
  Asides are the most-used component in any docs site. The starter ships four variants out of the
  box.
</Aside>

<Aside type="tip">Title is optional. If omitted, the variant name is used.</Aside>
...
<CardGrid>
  ...
  <Card title="Tabs">
    Multi-variant content for code samples, OS-specific instructions, or anything where the same
    content has several variants.
  </Card>
</CardGrid>
...
<Steps>
  ...
  <Step title="Ship">Run `astro build` and deploy the static output anywhere.</Step>
</Steps>
